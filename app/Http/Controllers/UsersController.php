...
class UsersController extends Controller
{
    ...
    public function followings($id)
    {
        ...
        if(\Auth::id() === $id) {
            ...
            // 「フォロー解除」したユーザが「フォロー中」から消えてしまうのを防ぐための対応。
            // 
            //  ログインユーザが、自分についてのユーザ詳細画面で「フォロー中」を表示時に、
            ...
            // 「return back()」時に、flashでのセッション値に指定している値の有無を見て
            // 値がない場合は、
            // タブ切替で「フォロー中」を表示したケースや、ページャーでのページ切り替え
            // のケースと判断つくため、そのときに、取得した
            // $followingsにとづいた並び順の情報も、セッション値に保存しておきたい
            // 「フォロー／フォロー解除」の操作中は、その並び順を維持したまま
            // 復元するため。
