...
class CreatePostsTable extends Migration
{
    ...
    public function up() //投稿する。
    {
        Schema::create('posts', function (Blueprint $table) { //マイグレーションは複数形。モデルは単数系。laravel側で自動判定。
            ...
            //$table->string('title')->nullable(); //タイトル（文字列）nullでも可。
            $table->text('content'); //投稿内容。X(twitter)でもtext型のため。
            ...
            //外部キー制約
            //$table->foreign('user_id')->references('id')->on('user')->onDelete('cascade');//ユーザーテーブルのidとポストテーブルのuser_idを外部キー制約。
        });
    }
