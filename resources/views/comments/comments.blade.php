    ...
    <ul class="list-unstyled">
        ...
        <li class="mb-3 text-center">
            <div class="text-left d-inline-block w-75 mb-2">
                ...
                <p class="mt-3 mb-0 d-inline-block">回答：<a href="{{ route('user.show', $comment->user->id) }}" class="text-black">{{$comment->user->name}}</a>さん</p>
                <p class="text-muted d-inline-block ml-4">{{$comment->created_at}}</p>
            </div>
            <div class="">
                <div class="text-left d-inline-block w-75">
                    <p class="text-center mb-2 h4">{{$posts->text}}</p>
                    <p class="text-center text-muted mb-2 h3 text-black">{{$comment->body}}</p>
                </div>
